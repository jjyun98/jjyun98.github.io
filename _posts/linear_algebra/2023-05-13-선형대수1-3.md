...
치환행렬 P에 대응되는 치환 $\Phi_{rc}(P)$가 k개의 호환들의 합성으로 표현되면, 치환행렬 P는 k개의 치환행렬들의 곱으로 표현된다.<br>
호환 $\tau{ij}$에 대해 $\Phi_{rc}^{-1}(\tau_{ij}) = R_{i,j} = \Phi_{cr}^{-1}(\tau_{ij})$가 성립하고 이로인해 단위행렬에서 두 행을 맞바꾼 행렬 $R_{i, j}$를 `호환행렬`이라 부른다. 또 호환행렬 $R_{i, j}$은 가역행렬이고, $R_{i, j}^{-1} = R_{i, j}^T$이므로 위 결과로부터 치환행렬 $P$가 가역이 되고 그 역행렬은 전치행렬 $P^T$가 됨을 확인할 수 있다.